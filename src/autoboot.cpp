...
void AutoBoot::handleCommand(const quint8 command, const quint8 aux1, const quint8 aux2) {
  ...
  switch (command) {
    ...
    case 0xFD: {
      ...
      qDebug() << "!n" << tr("[%1] Atari is jumping to %2.").arg(deviceName()).arg(aux1 + aux2 * 256);
      emit loaderDone();
      sio->port()->writeComplete();
      break;
    }
    case 0xFE: { /* Get chunk */
      ...
      qDebug() << "!n" << tr("[%1] Get chunk %2 (%3 bytes).").arg(deviceName()).arg(aux).arg(chunks.at(aux).data.size());
      sio->port()->writeComplete();
...
bool AutoBoot::open(const QString &fileName, bool highSpeed) {
  ...
  if (!boot.open(QFile::ReadOnly)) {
    qCritical() << "!e" << tr("Cannot open the boot loader: %1").arg(boot.errorString());
    return false;
  }
