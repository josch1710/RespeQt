...
namespace DiskImages {
  ...
  bool DiskImagePro::writeHappyTrack(int trackNumber, bool happy1050) {
    ...
    while (offset < (startOffset + 0x100)) {
      ...
      if (code < 128) {
        if (((quint8) m_board.m_happyRam[offset] == invertedTrack) && ((quint8) m_board.m_happyRam[offset + 1] == (quint8) 0xFF) && ((quint8) m_board.m_happyRam[offset + 2] >= 0xED) && ((quint8) m_board.m_happyRam[offset + 3] == (quint8) 0xFF) && ((quint8) m_board.m_happyRam[offset + 4] == (quint8) 0x08)) {
  ...
  bool DiskImagePro::writeHappySectors(int trackNumber, int afterSectorNumber, bool happy1050) {
    if (trackNumber != m_trackNumber) {
      QByteArray dummy;
      readTrack((quint16) trackNumber, dummy, 128);
    }
  ...
  bool DiskImagePro::resetTrack(quint16 aux) {
  ...
  quint8 DiskImagePro::writeSectorHeader(quint8 dataSize, quint16 sectorSlot, quint8 postDataCrc, quint8 preIDField, quint8 postIDCrc,
  ...
  bool DiskImagePro::writeSectorExtended(int, quint8 dataType, quint8 trackNumber, quint8, quint8 sectorNumber, quint8, const QByteArray &data, bool crcError, int weakOffset) {
  ...
  quint16 DiskImagePro::findPositionInTrack(int track, int indexInProSector, bool withoutData) {
    ...
    if (m_proSectorInfo[indexInProSector].weakBits == 0xFFFF) {
      ...
      for (int l = 0; l < 128 - 8; l++) {
        if (((quint8) m_proSectorInfo[indexInProSector].sectorData[l] == 0xFF) && ((quint8) m_proSectorInfo[indexInProSector].sectorData[l + 1] == (0xFF - DISK_ID_ADDR_MARK)) && ((quint8) m_proSectorInfo[indexInProSector].sectorData[l + 2] == invertedTrack)) {
          Crc16 crc16;
          crc16.Reset();
          for (int m = 0; m < 5; m++) {
  ...
  bool DiskImagePro::openImage(const QString &fileName) {
    QFile *sourceFile;
    ...
    // Validate the size
    quint16 magic = getBigEndianWord(header, 0);
    ...
    int numberOfSectors = getBigEndianWord(header, 6);
  ...
  bool DiskImagePro::create(int untitledName) {
  ...
  bool DiskImagePro::format(const DiskGeometry &geo) {
  ...
  bool DiskImagePro::readHappySectorAtPosition(int trackNumber, int sectorNumber, int afterSectorNumber, int &index, QByteArray &data) {
    data.resize(128);
    if (trackNumber != m_trackNumber) {
      QByteArray dummy;
      readTrack((quint16) trackNumber, dummy, 128);
  ...
  bool DiskImagePro::readHappySkewAlignment(bool happy1050) {
    ...
    readTrack(previousTrack, firstTrackData, 256);
    ...
    readTrack(currentTrack, secondTrackData, 256);
  ...
  void DiskImagePro::readTrack(quint16 aux, QByteArray &data, int length) {
  ...
  bool DiskImagePro::readSectorStatuses(QByteArray &data) {
    data.resize(128);
  ...
  bool DiskImagePro::readSectorUsingIndex(quint16 aux, QByteArray &data) {
    data.resize(128);
  ...
  //   Spy vs Spy 2
  //
  bool DiskImagePro::readSector(quint16 aux, QByteArray &data) {
  ...
  bool DiskImagePro::readSkewAlignment(quint16 aux, QByteArray &data, bool timingOnly) {
    // read sector list of the first track to compare to the list given by Super Archiver
    int firstTrack = aux & 0xFF;
    QByteArray firstTrackData;
    readTrack(firstTrack, firstTrackData, 256);
    int firstTrackSectorCount = m_sectorsInTrack;
    quint16 firstTrackContent[200];
    memcpy(firstTrackContent, m_trackContent, sizeof(firstTrackContent));
    ...
    QByteArray secondTrackData;
    readTrack(secondTrack, secondTrackData, 256);
  ...
  bool DiskImagePro::writeTrack(quint16 aux, const QByteArray &data) {
      ...
      for (quint8 index = 1; index <= m_board.m_chipRam[0]; index++) {
        ...
        if ((sector > 0) && (sector <= m_geometry.sectorsPerTrack())) {
          ...
          for (quint8 nextIndex = index; (dataSize < (quint8) 128) && (nextIndex < index + 10); nextIndex++) {
            ...
            dataSize = writeSectorHeader(dataSize, sectorSlot, postDataCrc, preIDField, postIDCrc, track, nextIndex, nextSector);
            if (dataSize < (quint8) 128) {
              Crc16 crc16;
              crc16.Reset();
              crc16.Add((unsigned char) DISK_DATA_ADDR_MARK4);
  ...
  bool DiskImagePro::writeTrackWithSkew(quint16 aux, const QByteArray &data) {
    // skew alignment not supported
    return writeTrack((aux & 0x3F) | 0xF000, m_board.m_trackData);
  }

  bool DiskImagePro::writeSectorUsingIndex(quint16 aux, const QByteArray &data, bool fuzzy) {
    quint16 chipFlags = m_board.isChipOpen() ? (aux >> 8) & 0xFC : 0;
    quint16 index = aux & 0x1F;
    int nbSectors = m_board.m_chipRam[0];
    if (nbSectors > 31) {
  ...
  bool DiskImagePro::writeFuzzySector(quint16 aux, const QByteArray &data) {
  ...
  bool DiskImagePro::writeSector(quint16 aux, const QByteArray &data) {
  ...
  int DiskImagePro::sectorsInCurrentTrack() {
    if (m_trackNumber <= 39) {
