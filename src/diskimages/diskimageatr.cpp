...
namespace DiskImages {
  ...
  bool SimpleDiskImage::openAtr(const QString &fileName) {
    ...
    //
    if (const auto imageSize {static_cast<quint64>(file.size())}; size != imageSize) {
      ...
      else
      {
          ...
          if (sizeValid) {
              if (sectorsWithoutPadding == 0) {
                  ...
                  const QByteArray emptySector{3 * 128, '\0'};
                  const auto imageSector{new char[3 * 128]};
                  ...
                  if (equals)
                      m_hasPadding = FilledAfterBootArea;
                  else {
    ...
    geometry.initialize(static_cast<uint>(size), secSize);
    ...
    if (!file.resize(static_cast<qint64>(size))) {
  ...
  bool SimpleDiskImage::openXfd(const QString &fileName) {
    ...
    const auto size {static_cast<quint64>(file.size())};
  ...
  bool SimpleDiskImage::saveAtr(const QString &fileName) {
    ...
    // Put signature
    m_originalFileHeader[0] = static_cast<char>(0x96);
    m_originalFileHeader[1] = 0x02;

    // Encode meta data
    m_originalFileHeader[2] = static_cast<char>((m_geometry.totalSize() >> 4) % 256);
    m_originalFileHeader[3] = static_cast<char>((m_geometry.totalSize() >> 12) % 256);
    m_originalFileHeader[4] = static_cast<char>(m_geometry.bytesPerSector() % 256);
    m_originalFileHeader[5] = static_cast<char>(m_geometry.bytesPerSector() / 256);
    m_originalFileHeader[6] = static_cast<char>((m_geometry.totalSize() >> 20) % 256);
    m_originalFileHeader[7] = static_cast<char>((m_geometry.totalSize() >> 28) % 256);
    ...
    while (file.bytesAvailable() != 0) {
      qint64 bufsize = 16777216;
      bufsize = std::min<qint64>(bufsize, file.bytesAvailable());
  ...
  bool SimpleDiskImage::saveXfd(const QString &fileName) {
    ...
    if (!guess.isEqual(m_geometry)) {
      if (guess.bytesPerSector() == m_geometry.bytesPerSector()) {
        qWarning() << "!w" << tr("Detailed disk geometry information will be lost when reopening '%1' due to XFD file format limitations.").arg(fileName);
      } else {
  ...
  bool SimpleDiskImage::saveAsAtr(const QString &fileName, const FileTypes::FileType destImageType) {
    ...
    for (quint16 sector = 1; sector <= m_geometry.sectorCount(); sector++) {
      ...
      if (const int sectorSize = sector <= 3 ? 128 : m_geometry.bytesPerSector(); data.size() < sectorSize) {
  ...
  bool SimpleDiskImage::createAtr(const int untitledName) {
    file.setFileTemplate(QDir::temp().absoluteFilePath("respeqt-temp-XXXXXX"));
    if (!file.open()) {
    ...
    m_geometry.initialize(static_cast<uint>(file.size()));
    refreshNewGeometry();
    m_originalFileHeader.clear();
    m_isReadOnly = false;
  ...
  bool SimpleDiskImage::readHappyAtrSectorAtPosition(const int trackNumber, const int sectorNumber, int, int &index, QByteArray &data) {
    // compute sector index for the sector number
    const int startHalf = (sectorNumber & 0x01) != 0 ? 0 : m_geometry.sectorsPerTrack() >> 1;
    const int indexInHalf = (sectorNumber - 1) >> 1;
  ...
  bool SimpleDiskImage::readHappyAtrSkewAlignment(const bool happy1050) {
    const quint8 previousTrack = static_cast<quint8>(0xFF - m_board.m_happyRam[0x3C9]);
    if (previousTrack > 39) {
    ...
    const quint8 currentTrack = static_cast<quint8>(0xFF - m_board.m_happyRam[0x3CB]);
    if (currentTrack > 39) {
    ...
    const quint8 previousSector = static_cast<quint8>(0xFF - m_board.m_happyRam[0x3CA]);
    ...
    const int previousIndex = startHalf + indexInHalf;
    const int previousPosition = sectorPositions[previousIndex];
    const quint8 currentSector = static_cast<quint8>(0xFF - m_board.m_happyRam[0x3CC]);
    ...
    const int currentIndex = startHalf + indexInHalf;
    const int currentPosition = sectorPositions[currentIndex];
  ...
  bool SimpleDiskImage::writeHappyAtrTrack(const int trackNumber, const bool happy1050) {
    ...
    while (offset < startOffset + 0x100) {
      const auto code = static_cast<quint8>(m_board.m_happyRam[offset++]);
      if (code == 0) {
        break;
      }
      if (code < 128) {
        nbSectors++;
        if (static_cast<quint8>(m_board.m_happyRam[offset++]) != invertedTrack) {
          notNormal = true;
          break;
        }
        if (static_cast<quint8>(m_board.m_happyRam[offset++]) != static_cast<quint8>(0xFF)) {
          notNormal = true;
          break;
        }
        if (static_cast<quint8>(m_board.m_happyRam[offset++]) < 0xED) {
          notNormal = true;
          break;
        }
        if (static_cast<quint8>(m_board.m_happyRam[offset++]) != static_cast<quint8>(0xFF)) {
        ...
        if (static_cast<quint8>(m_board.m_happyRam[offset++]) != static_cast<quint8>(0x08)) {
          notNormal = true;
          break;
        }
      } else {
  ...
  bool SimpleDiskImage::writeHappyAtrSectors(const int trackNumber, int, const bool happy1050) {
    ...
    while (index >= lastIndex) {
      ...
      if (sectorNumber > 0 && sectorNumber <= m_geometry.sectorsPerTrack()) {
        ...
        if (const auto writeCommand = static_cast<quint8>(m_board.m_happyRam[startOffset + 0x4A + index]); writeCommand != 0x57) {
          qWarning() << "!w" << tr("[%1] command $%2 not supported for sector %3 ($%4) with this kind of image. Ignored.").arg(deviceName()).arg(writeCommand).arg(sectorNumber).arg(sectorNumber, 2, 16, QChar('0'));
        }
      } else {
  ...
  bool SimpleDiskImage::seekToSector(const quint16 sector) {
    ...
    if (m_geometry.bytesPerSector() == 256
      ...
      if (sector <= 3 && m_hasPadding != SameSectorSize) {
        pos = (sector - 1) * 128;
      } else if (m_hasPadding == None) {
  ...
  void SimpleDiskImage::readAtrTrack(const quint16 aux, QByteArray &data, const int length) {
    m_trackNumber = aux & 0x3F;
    const bool useCount = aux & 0x40;
    const bool longHeader = aux & 0x8000;
  ...
  bool SimpleDiskImage::readAtrSectorStatuses(QByteArray &data) {
    ...
    for (int i = 0; i < nbSectors; i++) {
      QByteArray sector;
      readSectorUsingIndex(static_cast<quint16>(i), sector);
      char value = sector[0];
      ...
      data[i + 1] = static_cast<char>(0xFF);
      data[i + 0x41] = value;
  ...
  bool SimpleDiskImage::readAtrSector(const quint16 aux, QByteArray &data) {
    ...
    if (data.size() != m_geometry.bytesPerSector(sector)) {
      qCritical() << "!e" << tr("[%1] Cannot read from sector %2: %3.").arg(deviceName()).arg(sector).arg(file.errorString());
      return false;
    }
    return true;
  }

  bool SimpleDiskImage::readAtrSkewAlignment(const quint16 aux, const QByteArray &data, const bool timingOnly) {
    // skew alignment is not useful for ATR. Return default values
    m_board.m_trackData.clear();
    m_board.m_trackData.append(data);
    const auto track {static_cast<quint8>((aux >> 8) & 0xFF)};
    if (!timingOnly) {
      const quint16 *sectorPositions = m_geometry.sectorsPerTrack() == 26 ? ATX_SECTOR_POSITIONS_ED : ATX_SECTOR_POSITIONS_SD;
      const quint16 nbSectors = m_geometry.sectorsPerTrack();
      for (quint16 i = 0; i < nbSectors; i++) {
        // Assume standard interleave: 1, 3, 5, 7, 9, 11, 13, 15, 17, 2, 4, 6, 8, 10, 12, 14, 16, 18
        const quint16 sectorModulo = i % m_geometry.sectorsPerTrack();
        quint16 sectorIndex = static_cast<quint16>((sectorModulo + 1) << 1);
        ...
        m_board.m_trackData[0x48 + i] = static_cast<char>(sectorPosition & 0xFF);
      }
      ...
      if (track == 0) {
        // should not happen
        m_board.m_trackData[0xF4] = m_board.m_trackData[0xF5] = static_cast<char>(0xE8);
      } else {
        const quint16 timing = timings[(track - 1) % 14];
        m_board.m_trackData[0xF4] = static_cast<char>(timing & 0xFF);
        m_board.m_trackData[0xF5] = static_cast<char>((timing >> 8) & 0xFF);
      }
    }
    m_board.m_trackData[0] = static_cast<char>(m_deviceNo);
    m_board.m_trackData[1] = 0x74;
    m_board.m_trackData[2] = static_cast<char>(aux & 0xFF);
    m_board.m_trackData[3] = static_cast<char>(track);
    return true;
  }
  ...
  bool SimpleDiskImage::writeAtrTrack(const quint16 aux, const QByteArray &data) {
    m_trackNumber = aux & 0x3F;
    for (int i = 0; i < 32; i++) {
      m_board.m_chipRam[i] = static_cast<quint8>(data[i]);
    }
    const int absoluteSector = m_trackNumber * m_geometry.sectorsPerTrack();
    ...
    bool notNormal = false;
    if (const quint8 nbSector = static_cast<quint8>(data[0]); nbSector != m_geometry.sectorsPerTrack()) {
      notNormal = true;
    } else {
      for (int i = 0; i < nbSector; i++) {
        if (static_cast<quint8>(data[1 + i]) < 1 || static_cast<quint16>(data[1 + i]) > m_geometry.sectorsPerTrack()) {
          notNormal = true;
          break;
        }
        if (static_cast<quint8>(data[29 + i]) != 0) {
          notNormal = true;
          break;
        }
        if (static_cast<quint8>(data[85 + i]) != 0x80) {
          notNormal = true;
          break;
        }
      }
  ...
  bool SimpleDiskImage::writeAtrTrackWithSkew(const quint16 aux, const QByteArray &) {
    // skew alignment not supported
    return writeAtrTrack((aux & 0x3F) | 0xF000, m_board.m_trackData);
  }

  bool SimpleDiskImage::writeAtrSectorUsingIndex(const quint16 aux, const QByteArray &data, bool) {
    const quint16 index = aux & 0x1F;
  ...
  bool SimpleDiskImage::writeFuzzyAtrSector(const quint16 aux, const QByteArray &data) {
  ...
  bool SimpleDiskImage::writeAtrSector(const quint16 aux, const QByteArray &data) {
    const quint16 sector = m_board.isChipOpen() ? aux & 0x3FF : aux;
    if (!seekToSector(sector)) {
      return false;
    }
    ...
    if (file.write(data) != data.size()) {
      qCritical() << "!e" << tr("[%1] Cannot write to sector %2: %3.").arg(deviceName()).arg(sector).arg(file.errorString());
      return false;
    }
    return true;
  }

  bool SimpleDiskImage::writeAtrSectorExtended(int, quint8, const quint8 trackNumber, quint8, const quint8 sectorNumber, quint8, const QByteArray &data, bool, int) {
  ...
  int SimpleDiskImage::sectorsInCurrentAtrTrack() const
  {
    if (m_trackNumber <= 39) {
