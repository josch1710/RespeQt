...
namespace DiskImages {
    ...
    : SioDevice(worker)
  {
    ...
    m_toolDisk = nullptr;
    m_originalImageType = FileTypes::Unknown;
  }
  ...
  QString SimpleDiskImage::description() const {
    if (m_board.isToolDiskActive()) {
      ...
      if (m_activateHappyModeWithTool || m_board.isHappyEnabled()) {
        return QString(tr("Favorite tool disk")).append(" - ").append(tr("HAPPY mode"));
      }
      return tr("Favorite tool disk");
    }
    QString sides;
  ...
  void SimpleDiskImage::setChipMode(const bool enable) {
  ...
  void SimpleDiskImage::setHappyMode(const bool enable) {
    ...
    if (m_board.isHappyEnabled()) {
      ...
      if (m_board.m_happyRam.size() == 0) {
        ...
        for (unsigned int i = 0; i < sizeof(HAPPY_INITIAL_SECTOR_LIST); i++) {
          m_board.m_happyRam[0x280 + i] = static_cast<char>(HAPPY_INITIAL_SECTOR_LIST[i]);
        }
      }
    } else {
      m_board.m_happyRam[0] = static_cast<char>(0x80);
  ...
  void SimpleDiskImage::setOSBMode(const bool enable) {
    m_OSBMode = enable;
    setTranslatorActive(true);
  }

  void SimpleDiskImage::setTranslatorActive(const bool resetTranslatorState) {
    const bool oldState = m_board.isTranslatorActive();
  ...
  bool SimpleDiskImage::translatorDiskImageAvailable() {
    ...
    const auto translatorFile {new QFile(m_translatorDiskImagePath)};
  ...
  [[maybe_unused]] void SimpleDiskImage::setToolDiskMode(const bool enable) {
    m_toolDiskMode = enable;
    setToolDiskActive();
  }

  void SimpleDiskImage::setToolDiskActive() {
    const bool oldState = m_board.isToolDiskActive();
  ...
  bool SimpleDiskImage::toolDiskImageAvailable() {
    ...
    const auto toolDiskFile {new QFile(m_toolDiskImagePath)};
  ...
  void SimpleDiskImage::setDisplayTransmission(const bool active) {
    m_displayTransmission = active;
  }

  void SimpleDiskImage::setSpyMode(const bool enable) {
    m_dumpDataFrame = enable;
  }

  void SimpleDiskImage::setTrackLayout(const bool enable) {
    m_displayTrackLayout = enable;
  }

  void SimpleDiskImage::setDisassembleUploadedCode(const bool enable) {
    m_disassembleUploadedCode = enable;
  }

  void SimpleDiskImage::setTranslatorAutomaticDetection(const bool enable) {
  ...
  void SimpleDiskImage::setActivateChipModeWithTool(const bool activate) {
    m_activateChipModeWithTool = activate;
  }

  void SimpleDiskImage::setActivateHappyModeWithTool(const bool activate) {
    m_activateHappyModeWithTool = activate;
  }

  [[maybe_unused]] void SimpleDiskImage::setLeverOpen(const bool open) {
  ...
  bool SimpleDiskImage::openDcm(const QString &fileName) { // NOLINT(*-convert-member-functions-to-static)
    qCritical() << "!e" << tr("Cannot open '%1': %2").arg(fileName, tr("DCM images are not supported yet."));
    return false;
  }

  // ReSharper disable once CppMemberFunctionMayBeStatic
  bool SimpleDiskImage::openDi(const QString &fileName) { // NOLINT(*-convert-member-functions-to-static)
    qCritical() << "!e" << tr("Cannot open '%1': %2").arg(fileName, tr("DI images are not supported yet."));
    return false;
  }

  bool SimpleDiskImage::open(const QString &fileName, const FileTypes::FileType type) {
    ...
    if (bResult) {
      ...
      if (baseName.contains("Side", Qt::CaseInsensitive) || baseName.contains("Disk", Qt::CaseInsensitive)) {
        ...
        if (imageList.size() > 1) {
          ...
          const int nextIndex = m_currentSide >= m_numberOfSides ? 0 : m_currentSide;
          m_nextSideFilename.append(imageList.at(nextIndex));
          const QFileInfo nextFileInfo(m_nextSideFilename);
  ...
  // ReSharper disable once CppMemberFunctionMayBeStatic
  bool SimpleDiskImage::sameSoftware(const QString &fileName, const QString &otherFileName) { // NOLINT(*-convert-member-functions-to-static)
    int same = 0;
    const int minLength = fileName.length() < otherFileName.length() ? fileName.length() : otherFileName.length();
    QChar c1, c2;
    ...
    if (const QString samePart = fileName.left(same);
      !samePart.contains("Side", Qt::CaseInsensitive) && !samePart.contains("Disk", Qt::CaseInsensitive)
    ) {
      return false;
    }
    return true;
  ...
  // ReSharper disable once CppMemberFunctionMayBeStatic
  bool SimpleDiskImage::saveDcm(const QString &fileName) { // NOLINT(*-convert-member-functions-to-static)
    qCritical() << "!e" << tr("Cannot save '%1': %2").arg(fileName, tr("Saving DCM images is not supported yet."));
    return false;
  }

  // ReSharper disable once CppMemberFunctionMayBeStatic
  bool SimpleDiskImage::saveDi(const QString &fileName) { // NOLINT(*-convert-member-functions-to-static)
  ...
  [[maybe_unused]] bool SimpleDiskImage::saveAs(const QString &fileName) {
    m_currentSide = 1;
    m_numberOfSides = 1;
    m_nextSideFilename.clear();
    if (fileName.endsWith(".ATR", Qt::CaseInsensitive)) {
      constexpr FileTypes::FileType destinationImageType = FileTypes::Atr;
    ...
    if (fileName.endsWith(".ATZ", Qt::CaseInsensitive) || fileName.endsWith(".ATR.GZ", Qt::CaseInsensitive)) {
      constexpr FileTypes::FileType destinationImageType = FileTypes::AtrGz;
      if (m_originalImageType == FileTypes::Atr || m_originalImageType == FileTypes::AtrGz || m_originalImageType == FileTypes::Xfd || m_originalImageType == FileTypes::XfdGz) {
        // almost same format but different name
        m_originalImageType = destinationImageType;
        return saveAtr(fileName);
    ...
    if (fileName.endsWith(".XFD", Qt::CaseInsensitive)) {
      constexpr FileTypes::FileType destinationImageType = FileTypes::Xfd;
      if (m_originalImageType == FileTypes::Atr || m_originalImageType == FileTypes::AtrGz || m_originalImageType == FileTypes::Xfd || m_originalImageType == FileTypes::XfdGz) {
        // almost same format but different name
        m_originalImageType = destinationImageType;
    ...
    if (fileName.endsWith(".XFZ", Qt::CaseInsensitive) || fileName.endsWith(".XFD.GZ", Qt::CaseInsensitive)) {
      constexpr FileTypes::FileType destinationImageType = FileTypes::XfdGz;
      if (m_originalImageType == FileTypes::Atr || m_originalImageType == FileTypes::AtrGz || m_originalImageType == FileTypes::Xfd || m_originalImageType == FileTypes::XfdGz) {
        // almost same format but different name
        m_originalImageType = destinationImageType;
        return saveXfd(fileName);
    ...
    if (fileName.endsWith(".DI", Qt::CaseInsensitive)) {
      m_originalImageType = FileTypes::Di;
      return saveDi(fileName);
    }
    if (fileName.endsWith(".PRO", Qt::CaseInsensitive)) {
      constexpr FileTypes::FileType destinationImageType = FileTypes::Pro;
    ...
    if (fileName.endsWith(".PRO.GZ", Qt::CaseInsensitive)) {
      constexpr FileTypes::FileType destinationImageType = FileTypes::ProGz;
      if (m_originalImageType == FileTypes::Pro || m_originalImageType == FileTypes::ProGz) {
        // same format but different name
        m_originalImageType = destinationImageType;
        return savePro(fileName);
    ...
    if (fileName.endsWith(".ATX", Qt::CaseInsensitive)) {
      constexpr FileTypes::FileType destinationImageType = FileTypes::Atx;
    ...
    if (fileName.endsWith(".ATX.GZ", Qt::CaseInsensitive)) {
      constexpr FileTypes::FileType destinationImageType = FileTypes::AtxGz;
      if (m_originalImageType == FileTypes::Atx || m_originalImageType == FileTypes::AtxGz) {
        // same format but different name
        m_originalImageType = destinationImageType;
        return saveAtx(fileName);
  ...
  bool SimpleDiskImage::create(const int untitledName) {
    if (m_originalImageType == FileTypes::Pro || m_originalImageType == FileTypes::ProGz) {
  ...
  [[maybe_unused]] void SimpleDiskImage::reopen() {
    close();
  ...
  void SimpleDiskImage::setReady(const bool bReady) {
    m_isReady = bReady;
  }

  bool SimpleDiskImage::executeArchiverCode(const quint16 aux, QByteArray &data) {
    bool ok = true;
    if (static_cast<quint8>(data[0]) == 0x18 && static_cast<quint8>(data[1]) == 0x60) {// Super Archiver 3.12 command to check if chip is open
      ...
      for (int j = 0; j < data.size(); j++) {
        crc16.Add(static_cast<unsigned char>(data.at(j)));
        if (j == 12) {
      ...
      if (m_board.getLastArchiverUploadCrc16() == 0xFD2E) {
        ...
        const int startOffset = static_cast<int>(data[3]) & 0xFF;// this byte contains the offset of the first track data byte
        ...
        if (crc16.GetCrc() == 0x603D || crc16.GetCrc() == 0xBAC2 || crc16.GetCrc() == 0xDFFF) {// Super Archiver 3.02, 3.03, 3.12 respectively
          ...
          const bool timingOnly = crc16.GetCrc() == 0xDFFF;
          if (timingOnly) {
  ...
  void SimpleDiskImage::readHappyTrack(const int trackNumber, const bool happy1050) {
    ...
    nbSectors = std::min<quint8>(nbSectors, static_cast<quint8>(nbSectorsInTrack + 1));
    int srcOffset = 1;
    int startOffset;
    int dstOffset;
    if (happy1050) {
      ...
      m_board.m_happyRam[dstOffset++] = static_cast<char>(0x8D);
      m_board.m_happyRam[dstOffset++] = 0x7F;
    } else {
      startOffset = 0x300;
      dstOffset = startOffset + 1;
      m_board.m_happyRam[dstOffset++] = 0x0B;
      m_board.m_happyRam[dstOffset++] = static_cast<char>(0xFE);
    }
    quint8 totalTiming = 0;
    for (quint8 sector = 0; sector < nbSectors; sector++) {
      // ignore short sectors
      if (static_cast<unsigned char>(data[srcOffset + 4]) >= 2) {
        crc.Reset();
        crc.Add(DISK_ID_ADDR_MARK);
        for (int i = 0; i < 4; i++) {
          // ReSharper disable once CppRedundantCastExpression
          m_board.m_happyRam[dstOffset++] = static_cast<char>(0xFF - crc.Add(static_cast<unsigned char>(data[srcOffset++])));
        }
    ...
    if (!happy1050) {
      m_board.m_happyRam[0x28D] = 0x43;
    }
  }

  QByteArray SimpleDiskImage::readHappySectors(const int trackNumber, const int afterSectorNumber, const bool happy1050) {
    const int baseOffset = happy1050 ? 0xC80 : 0x280;
    const bool rev50 = happy1050 ? false : static_cast<quint8>(m_board.m_happyRam[0x14D]) == 0x18;
    const quint8 fdcMask = rev50 ? 0x18 : static_cast<quint8>(m_board.m_happyRam[baseOffset + 0x01]);
    // ReSharper disable once CppRedundantCastExpression
    quint8 nbSectors = static_cast<quint8>(m_board.m_happyRam[baseOffset + 0x0F]);
    int start = m_board.m_happyRam[baseOffset + 0x12];
    start = std::max(std::min(start, 35), 0);
    int nbRetry = 4;
    ...
    do {
      ...
      for (int i = start; i >= 0; i--) {
        if ((0xFF - m_board.m_happyRam[baseOffset + 0x14 + i] & fdcMask) != 0) {
          ...
          m_board.m_happyRam[baseOffset + 0x14 + i] = static_cast<char>(m_wd1771Status);
          nbSectors--;
          if (nbSectors <= 0) {
            break;
          }
  ...
  bool SimpleDiskImage::readHappySectorAtPosition(const int trackNumber, const int sectorNumber, const int afterSectorNumber, int &index, QByteArray &data) {
    if (m_originalImageType == FileTypes::Pro || m_originalImageType == FileTypes::ProGz) {
  ...
  bool SimpleDiskImage::readHappySkewAlignment(const bool happy1050) {
    if (m_originalImageType == FileTypes::Pro || m_originalImageType == FileTypes::ProGz) {
  ...
  bool SimpleDiskImage::writeHappyTrack(const int trackNumber, const bool happy1050) {
    if (m_originalImageType == FileTypes::Pro || m_originalImageType == FileTypes::ProGz) {
  ...
  bool SimpleDiskImage::writeHappySectors(const int trackNumber, const int afterSectorNumber, const bool happy1050) {
    if (m_originalImageType == FileTypes::Pro || m_originalImageType == FileTypes::ProGz) {
  ...
  void SimpleDiskImage::readTrack(const quint16 aux, QByteArray &data, const int length) {
    if (m_originalImageType == FileTypes::Pro || m_originalImageType == FileTypes::ProGz) {
  ...
  bool SimpleDiskImage::readSectorUsingIndex(const quint16 aux, QByteArray &data) {
    if (m_originalImageType == FileTypes::Pro || m_originalImageType == FileTypes::ProGz) {
  ...
  bool SimpleDiskImage::readSector(const quint16 aux, QByteArray &data) {
    if (m_originalImageType == FileTypes::Pro || m_originalImageType == FileTypes::ProGz) {
  ...
  bool SimpleDiskImage::readSkewAlignment(const quint16 aux, QByteArray &data, const bool timingOnly) {
    if (m_originalImageType == FileTypes::Pro || m_originalImageType == FileTypes::ProGz) {
  ...
  [[maybe_unused]] bool SimpleDiskImage::resetTrack(const quint16 aux) {
    if (!m_isModified) {
      m_isModified = true;
      emit statusChanged(m_deviceNo);
    }
  ...
  bool SimpleDiskImage::writeTrack(const quint16 aux, const QByteArray &data) {
    if (m_originalImageType == FileTypes::Pro || m_originalImageType == FileTypes::ProGz) {
  ...
  bool SimpleDiskImage::writeTrackWithSkew(const quint16 aux, const QByteArray &data) {
    if (m_originalImageType == FileTypes::Pro || m_originalImageType == FileTypes::ProGz) {
  ...
  bool SimpleDiskImage::writeSectorUsingIndex(const quint16 aux, const QByteArray &data, const bool fuzzy) {
    if (m_originalImageType == FileTypes::Pro || m_originalImageType == FileTypes::ProGz) {
  ...
  bool SimpleDiskImage::writeFuzzySector(const quint16 aux, const QByteArray &data) {
    if (m_originalImageType == FileTypes::Pro || m_originalImageType == FileTypes::ProGz) {
  ...
  bool SimpleDiskImage::writeSector(const quint16 aux, const QByteArray &data) {
    if (m_originalImageType == FileTypes::Pro || m_originalImageType == FileTypes::ProGz) {
  ...
  [[maybe_unused]] bool SimpleDiskImage::writeSectorExtended(const int bitNumber, const quint8 dataType, const quint8 trackNumber, const quint8 sideNumber, const quint8 sectorNumber, const quint8 sectorSize, const QByteArray &data, const bool crcError, const int weakOffset) {
    if (m_originalImageType == FileTypes::Pro || m_originalImageType == FileTypes::ProGz) {
  ...
  // ReSharper disable once CppMemberFunctionMayBeStatic
  constexpr unsigned int SimpleDiskImage::findNearestSpeed(const unsigned int speed) const
  {
    if (speed < 20000) {
      // ReSharper disable once CppRedundantCastExpression
      return static_cast<unsigned int>(19200);
    }
  ...
  void SimpleDiskImage::handleCommand(const quint8 command, const quint8 aux1, const quint8 aux2) {
      ...
      if (m_board.isTranslatorActive()) {
        if (sector == 1) {
          if (m_board.getTranslatorState() == BOOT_STATE::NOT_BOOTED) {
            ...
            if (FileTypes::FileType type = FileTypes::getFileType(m_translatorDiskImagePath); !m_translatorDisk->open(m_translatorDiskImagePath, type)) {
        ...
        if (m_toolDisk == nullptr) {
          ...
          if (FileTypes::FileType type = FileTypes::getFileType(m_toolDiskImagePath); !m_toolDisk->open(m_toolDiskImagePath, type)) {
    ...
    switch (command) {
      ...
      case 0x22:// Format ED
      {
        ...
        data[0] = data[1] = static_cast<char>(0xFF);
        if (m_isReadOnly) {
          writeError();
      ...
      case 0x24:// Run Diagnostic (1050)
      {
        ...
        if (static_cast<quint8>(m_diagData[0]) == static_cast<quint8>(0x01) && static_cast<quint8>(m_diagData[1]) == static_cast<quint8>(0x9B)) {
          ...
          m_diagData[125] = 0x00;
          m_diagData[126] = 0x56;
          m_diagData[127] = static_cast<char>(0xC6);
        } else if (static_cast<quint8>(m_diagData[0]) == static_cast<quint8>(0x00) && static_cast<quint8>(m_diagData[1]) == static_cast<quint8>(0x00)) {
          m_diagData.resize(122);
          m_diagData[0] = 0x22;
          m_diagData[1] = 0x08;
          m_diagData[122] = 0x14;
      ...
      case 0x3F:// Speed poll
      {
        ...
        qDebug() << "!n" << tr("[%1] Speed poll: %2").arg(deviceName()).arg(static_cast<int>(speed[0]));
        writeComplete();
      ...
      case 0x42:// Write Sector using Index (CHIP/ARCHIVER) or Read All Sectors (HAPPY Rev.7)
      {
        ...
        if (QByteArray data = readDataFrame(m_geometry.bytesPerSector()); !data.isEmpty()) {
          if (!writeDataAck()) {
            break;
          }
          if (m_isReadOnly) {
      ...
      case 0x43:// Read all Sector Statuses (CHIP/ARCHIVER) or Set Skew Alignment (HAPPY Rev.7)
      {
        ...
        if (QByteArray data; readSectorStatuses(data)) {
          if (!writeComplete()) {
            break;
          }
          writeDataFrame(data);
      ...
      case 0x44:// Read Sector using Index (CHIP/ARCHIVER) or Read Skew Alignment (HAPPY Rev.7)
      {
        ...
        if (QByteArray data; readSectorUsingIndex(aux, data)) {
          if (!writeComplete()) {
            break;
          }
          writeDataFrame(data);
      ...
      case 0x49:// Write track with skew alignment (HAPPY Rev.7)
      {
        if (m_board.isHappyEnabled() && m_board.isHappy1050()) {
          if (m_board.getLastHappyUploadCrc16() == 0x4312) {
            ...
            int trackNumber = 0xFF - aux1;
            qDebug() << "!n" << tr("[%1] Happy Write track using skew alignment of track %2 ($%3) with track %4 ($%5) sector %6 ($%7)").arg(deviceName()).arg(trackNumber).arg(trackNumber, 2, 16, QChar('0')).arg(static_cast<quint8>(0xFF) - static_cast<quint8>(m_board.m_happyRam[0x3CB])).arg(static_cast<quint8>(0xFF) - static_cast<quint8>(m_board.m_happyRam[0x3CB]), 2, 16, QChar('0')).arg(static_cast<quint8>(0xFF) - static_cast<quint8>(m_board.m_happyRam[0x3CC])).arg(static_cast<quint8>(0xFF) - static_cast<quint8>(m_board.m_happyRam[0x3CC]), 2, 16, QChar('0'));
            QThread::usleep(150);
            sio->port()->setSpeed(findNearestSpeed(38400) + 1);// +1 (odd number) is a trick to set 2 stop bits (needed by Happy 810)
            if (writeHappyTrack(trackNumber, true)) {
              writeComplete();
      ...
      case 0x4C:// Set RAM Buffer (CHIP/ARCHIVER)
      {
        ...
        if (QByteArray ram = readDataFrame(128); ram.size() == 128) {
          if (!writeDataAck()) {
            break;
          }
          writeComplete();
          for (int i = 0; i < 32; i++) {
            m_board.m_chipRam[i] = static_cast<unsigned char>(ram[i]);
          }
        } else {
      ...
      case 0x77:// High Speed Write sector without verify or Write memory (HAPPY 1050)
      {
        ...
        if (m_board.isChipOpen()) {
          ...
          if (command == 0x70) {
            ...
            writeDataFrame(QByteArray(reinterpret_cast<const char*>(ARCHIVER_SPEED_CHECK), sizeof(ARCHIVER_SPEED_CHECK)));
            break;
          }
          ...
          if (!m_board.isHappy1050() && aux >= 0x0800 && aux <= 0x1380) {// Write memory (HAPPY 810)
            ...
            if (QByteArray ram = readDataFrame(m_geometry.bytesPerSector(sector)); !ram.isEmpty()) {
              if (!writeDataAck()) {
                break;
              }
              ...
              if (aux == 0x0800) {
                ...
                for (int j = 0; j < ram.size(); j++) {
                  crc16.Add(static_cast<unsigned char>(ram.at(j)));
                }
                m_board.setLastHappyUploadCrc16(crc16.GetCrc());
                m_remainingBytes.clear();
              }
              if (m_disassembleUploadedCode) {
                if (int address = getUploadCodeStartAddress(command, aux, ram); address != -1) {
                  disassembleCode(ram, static_cast<unsigned short>(address), false, m_board.hasHappySignature());
                }
              }
              writeComplete();
            } else {
          ...
          if (aux & 0x8000 && aux < 0x9800) {// Write memory (HAPPY Rev.7)
            if (!writeCommandAck()) {
              break;
            }
            m_board.setHappy1050(true);
            ...
            if (QByteArray ram = readDataFrame(m_geometry.bytesPerSector(sector)); !ram.isEmpty()) {
              ...
              if (m_disassembleUploadedCode) {
                if (int address = getUploadCodeStartAddress(command, aux, ram); address != -1) {
                  disassembleCode(ram, static_cast<unsigned short>(address), true, true);
                }
              }
              writeComplete();
            } else {
        ...
        if (sector >= 1 && sector <= m_geometry.sectorCount()) {
          ...
          // A patched version of Happy Warp Speed Software V7.1.atr has been prepared to send data at normal speed to RespeQt
          // The original version is not compatible with RespeQt.
          if (QByteArray data = readDataFrame(m_geometry.bytesPerSector(sector)); !data.isEmpty()) {
            if (!writeDataAck()) {
              break;
            }
            if (m_isReadOnly) {
      ...
      case 0x51:// Execute custom code (HAPPY)
      {
        if (m_board.isHappyEnabled() && m_board.hasHappySignature()) {
          ...
          if (m_board.getLastHappyUploadCrc16() == 0xEF1A || m_board.getLastHappyUploadCrc16() == 0xC8C8) {// Happy Rev.5 and Rev.7
            ...
            writeDataFrame(QByteArray(reinterpret_cast<const char*>(HAPPY_RAM_CHECK), sizeof(HAPPY_RAM_CHECK)));
          } else if (m_board.getLastHappyUploadCrc16() == 0xC545) {
            qDebug() << "!n" << tr("[%1] Happy Rom 810 test").arg(deviceName());
            writeDataFrame(QByteArray(reinterpret_cast<const char*>(HAPPY_ROM810_CHECK), sizeof(HAPPY_ROM810_CHECK)));
          } else if (m_board.getLastHappyUploadCrc16() == 0x72CD) {
            qDebug() << "!n" << tr("[%1] Happy Extended ram check").arg(deviceName());
            writeDataFrame(QByteArray(reinterpret_cast<const char*>(HAPPY_RAM_EXTENDED_CHECK), sizeof(HAPPY_RAM_EXTENDED_CHECK)));
          } else if (m_board.getLastHappyUploadCrc16() == 0xEABE) {
            qDebug() << "!n" << tr("[%1] Happy Run diagnostic").arg(deviceName());
            writeDataFrame(QByteArray(reinterpret_cast<const char*>(HAPPY_DIAGNOSTIC), sizeof(HAPPY_DIAGNOSTIC)));
          } else if (m_board.getLastHappyUploadCrc16() == 0x8735) {
            qDebug() << "!n" << tr("[%1] Happy Speed check").arg(deviceName());
            writeDataFrame(QByteArray(reinterpret_cast<const char*>(HAPPY_SPEED_CHECK), sizeof(HAPPY_SPEED_CHECK)));
            ...
            writeDataFrame(QByteArray(reinterpret_cast<const char*>(HAPPY_RW_CHECK), sizeof(HAPPY_RW_CHECK)));
            ...
            writeDataFrame(QByteArray(reinterpret_cast<const char*>(HAPPY_BACKUP_BUF1), sizeof(HAPPY_BACKUP_BUF1)));
          } else if (m_board.getLastHappyUploadCrc16() == 0xCC68) {
            qDebug() << "!n" << tr("[%1] Happy init backup 2").arg(deviceName());
            writeDataFrame(QByteArray(reinterpret_cast<const char*>(HAPPY_BACKUP_BUF2), sizeof(HAPPY_BACKUP_BUF2)));
          } else {
      ...
      case 0x72:// High Speed Read sector or Read memory (HAPPY 1050)
      {
        ...
        if (m_board.isChipOpen()) {
          ...
          if (aux & 0x8000 && aux < 0x9800) {// Read memory (HAPPY Rev.7)
            if (!writeCommandAck()) {
              break;
            }
        ...
        if (sector >= 1 && sector <= m_geometry.sectorCount()) {
          ...
          if (QByteArray data; readSector(aux, data)) {
            if (quint16 chipFlags = m_board.isChipOpen() ? aux & 0xFC00 : 0; m_wd1771Status != 0xFF && chipFlags == 0) {
              if (!writeError()) {
                break;
              }
            } else {
              ...
              if (m_board.isHappyEnabled() && m_deviceNo == 0x31) {
                if (command == 0x72 && (sector == 397 || sector == 421)) {
                  ...
                  if (sector == 397 && crc16.GetCrc() == 0x7102) {
                    writeDataFrame(QByteArray(reinterpret_cast<const char*>(HAPPY_DOUBLESPEEDREV7_PATCH1), sizeof(HAPPY_DOUBLESPEEDREV7_PATCH1)));
                  ...
                  if (sector == 421 && crc16.GetCrc() == 0xF00A && m_board.isHappyPatchInProgress()) {
                    writeDataFrame(QByteArray(reinterpret_cast<const char*>(HAPPY_DOUBLESPEEDREV7_PATCH2), sizeof(HAPPY_DOUBLESPEEDREV7_PATCH2)));
      ...
      case 0x53:// Get status
      {
        ...
        if (quint16 aux = aux1 + aux2 * 256; aux == 0xABCD) {
          ...
          qDebug() << "!n" << tr("[%1] Get status: $%2").arg(deviceName()).arg(status[1], 2, 16, QChar('0'));
        }
        writeComplete();
      ...
      case 0x54:// Get RAM Buffer (CHIP/ARCHIVER) or Write sector and Write track (Happy 810 Rev.5)
      {
        if (m_board.isHappyEnabled()) {
          if (m_board.hasHappySignature())
          {
            ...
            if (m_board.getLastHappyUploadCrc16() == 0xC96C) {
              if (!writeCommandAck()) {
                break;
              }
              int trackNumber = 0xFF - aux1;
            ...
            if (crc16.GetCrc() == 0x544B) {
              ...
              writeDataFrame(QByteArray(reinterpret_cast<const char*>(HAPPY_ROM1050_CHECK), sizeof(HAPPY_ROM1050_CHECK)));
              break;
            }
          }
        }
        ...
        QByteArray result(128, 0);
        for (int i = 0; i < 32; i++) {
          result[i] = static_cast<char>(m_board.m_chipRam[i]);
        }
        writeDataFrame(result);
        break;
      }
      ...
      case 0x56:// Execute custom code (HAPPY)
      {
        if (m_board.isHappyEnabled() && m_board.hasHappySignature()) {
          ...
          if (command == 0x55 && m_board.getLastHappyUploadCrc16() == 0x4416) {
            ...
            int trackNumber = static_cast<quint8>(0xFF) - static_cast<quint8>(m_board.m_happyRam[0x84F]);
            int sectorNumber = static_cast<quint8>(0xFF) - static_cast<quint8>(m_board.m_happyRam[0x81A]);
            qDebug() << "!n" << tr("[%1] Happy Write track %2 ($%3) skew aligned with track %4 ($%5) sector %6 ($%7)").arg(deviceName()).arg(trackNumber).arg(trackNumber, 2, 16, QChar('0')).arg(static_cast<quint8>(0xFF) - static_cast<quint8>(m_board.m_happyRam[0x816])).arg(static_cast<quint8>(0xFF) - static_cast<quint8>(m_board.m_happyRam[0x816]), 2, 16, QChar('0')).arg(sectorNumber).arg(sectorNumber, 2, 16, QChar('0'));
            if (writeHappyTrack(trackNumber, false)) {
              writeComplete();
            } else {
              writeError();
      ...
      case 0x74:// Read Memory (ARCHIVER)
      {
        ...
        if (m_board.getLastArchiverUploadCrc16() == 0xFD2E) {
          ...
          ARCHIVER_SPEED_CHECK[2] = static_cast<quint8>(m_board.getLastArchiverSpeed());
        ...
        // this is the code for the diagnostic test (Archiver memory, 6532, 6810, Archiver version and rom checksum)
          ...
          writeDataFrame(QByteArray(reinterpret_cast<const char*>(ARCHIVER_DIAGNOSTIC), sizeof(ARCHIVER_DIAGNOSTIC)));
        }
        // this is the code for reading the address marks
          ...
          for (quint16 i = 0; i < sizeof(ARCHIVER_ADDRESS_CHECK); i++) {
            result[i] = static_cast<char>(ARCHIVER_ADDRESS_CHECK[i]);
          }
          result[2] = static_cast<char>(aux & 0x3F);
          for (quint16 i = 0; i < 256 - sizeof(ARCHIVER_ADDRESS_CHECK); i++) {
            result[i + static_cast<quint16>(sizeof(ARCHIVER_ADDRESS_CHECK))] = data[i + 4];
          }
          result[8] = data[3];
          writeDataFrame(result);
        }
      ...
      case 0x75:// Upload and execute code (ARCHIVER)
      {
        ...
        if (QByteArray data = readDataFrame(256); !data.isEmpty()) {
          if (!writeDataAck()) {
            break;
          }
          quint16 aux = aux1 + aux2 * 256;
          ...
          if (executeArchiverCode(aux, data)) {
            writeComplete();
          } else {
            writeError();
          }
  ...
  void SimpleDiskImage::getStatus(QByteArray &status) {
    constexpr quint8 motorSpinning = 0x10;
  ...
  quint8 SimpleDiskImage::computeVersionByte() {
    constexpr auto str = VERSION;
    int index = 0;
    if (str[index] == 'r') {
      index++;
    }
    const int major = str[index++] - '0';
    int minor = 0;
    if (str[index] == '.') {
      index++;
      minor = str[index] - '0';
    }
    return static_cast<quint8>((major << 4) + minor);
  }
  ...
  int SimpleDiskImage::defaultFileSystem() {
    ...
    if (m_geometry.bytesPerSector() == 512) {
      if (static_cast<quint8>(data.at(32)) == 0x21) {
        return 5;
      }
      return 0;
    }
    if (static_cast<quint8>(data.at(7)) == 0x80 /*&& ((quint8)data.at(32) == 0x21 || (quint8)data.at(32) == 0x20)*/) {
      return 5;
    }
    ...
    if ((m_geometry.isStandardSD() || m_geometry.isStandardDD()) && v == 2 && s == 707 && f <= s) {
      return 2;
    }
    if (m_geometry.isStandardED() && v == 2 && s == 1010 && f <= s) {
      return 3;
    }
    if (v == 2 && s + 12 == m_geometry.sectorCount() && f <= s) {
      return 4;
    }
    if (m_geometry.bytesPerSector() == 128 && s + (v * 2 - 3) + 10 == m_geometry.sectorCount() && f <= s) {
      return 4;
    }
    if (m_geometry.bytesPerSector() == 256 && s + v + 9 == m_geometry.sectorCount() && f <= s) {
      return 4;
    }
    return 0;
  }
  ...
  QByteArray SimpleDiskImage::readDataFrame(const uint size) {
    ...
    if (m_dumpDataFrame) {
      ...
      dumpBuffer(reinterpret_cast<unsigned char*>(data.data()), data.length());
    }
    return data;
  }
  ...
  // ReSharper disable once CppMemberFunctionMayBeStatic
  void SimpleDiskImage::setLittleEndianLong(QByteArray &array, const int offset, const quint32 value) {
    array[offset] = static_cast<char>(value & 0xFF);
    array[offset + 1] = static_cast<char>((value >> 8) & 0xFF);
    array[offset + 2] = static_cast<char>((value >> 16) & 0xFF);
    array[offset + 3] = static_cast<char>((value >> 24) & 0xFF);
  }
    ...
    if (len - ofs >= 16) {
      if (dumpAscii) {
        ...
        snprintf(line, 75, "$%04X: %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X | %c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c",
                 ofs & 0xFFFF, static_cast<unsigned int>(buf[ofs + 0]) & 0xFF, static_cast<unsigned int>(buf[ofs + 1]) & 0xFF,
                 static_cast<unsigned int>(buf[ofs + 2]) & 0xFF, static_cast<unsigned int>(buf[ofs + 3]) & 0xFF, static_cast<unsigned int>(buf[ofs + 4]) & 0xFF,
                 static_cast<unsigned int>(buf[ofs + 5]) & 0xFF, static_cast<unsigned int>(buf[ofs + 6]) & 0xFF, static_cast<unsigned int>(buf[ofs + 7]) & 0xFF,
                 static_cast<unsigned int>(buf[ofs + 8]) & 0xFF, static_cast<unsigned int>(buf[ofs + 9]) & 0xFF, static_cast<unsigned int>(buf[ofs + 10]) & 0xFF,
                 static_cast<unsigned int>(buf[ofs + 11]) & 0xFF, static_cast<unsigned int>(buf[ofs + 12]) & 0xFF, static_cast<unsigned int>(buf[ofs + 13]) & 0xFF,
                 static_cast<unsigned int>(buf[ofs + 14]) & 0xFF, static_cast<unsigned int>(buf[ofs + 15]) & 0xFF, car[0], car[1], car[2], car[3],
        ...
        snprintf(line, 55, "$%04X: %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X",
                 ...
                 static_cast<unsigned int>(buf[ofs + 8]) & 0xFF, static_cast<unsigned int>(buf[ofs + 9]) & 0xFF, static_cast<unsigned int>(buf[ofs + 10]) & 0xFF,
                 static_cast<unsigned int>(buf[ofs + 11]) & 0xFF, static_cast<unsigned int>(buf[ofs + 12]) & 0xFF, static_cast<unsigned int>(buf[ofs + 13]) & 0xFF,
                 static_cast<unsigned int>(buf[ofs + 14]) & 0xFF, static_cast<unsigned int>(buf[ofs + 15]) & 0xFF);
      }
    } else if (ofs < len) {
      const int nbRemaining = len - ofs;
      memset(line, ' ', 73);
      line[73] = 0;
      snprintf(line, 7, "$%04X:", ofs);
      for (int i = 0; i < nbRemaining; i++) {
        snprintf(&line[strlen(line)], 4, " %02X", static_cast<unsigned int>(buf[ofs + i]) & 0xFF);
      }
      if (dumpAscii) {
        for (auto i = strlen(line); i < 54; i++) {
          line[i] = ' ';
        }
        strcpy(&line[54], " | ");
        for (int i = 0; i < nbRemaining; i++) {
          if (buf[ofs + i] > 32 && buf[ofs + i] < 127) {
            line[57 + i] = static_cast<char>(buf[ofs + i]);
  ...
  void SimpleDiskImage::dumpBuffer(const unsigned char *buf, const int len) {
  ...
  int SimpleDiskImage::getUploadCodeStartAddress(const quint8 command, const quint16 aux, QByteArray &) {
    if (m_board.isChipOpen()) {
      if (command == 0x4D) {
        return 0x0080;
      }
  ...
  void SimpleDiskImage::disassembleCode(QByteArray &data, unsigned short address, const bool drive1050, const bool happy) {
    ...
    int len = data.size();
    for (int i = 0; i < len; i++) {
      crc.Add(static_cast<unsigned char>(data[i]));
    }
    ...
    if (happy) {
      if (address == 0x0800) {
        while (offset < 9) {
          qDebug() << "!u" << tr("[%1] §$%2: %3 %4 %5 ; Happy signature").arg(deviceName()).arg(address + static_cast<unsigned short>(offset), 4, 16, QChar('0')).arg(data[offset], 2, 16, QChar('0')).arg(data[offset + 1], 2, 16, QChar('0')).arg(data[offset + 2], 2, 16, QChar('0'));
          offset += 3;
        }
        int command = 0x50;
        while (offset < 32) {
          qDebug() << "!u" << tr("[%1] §$%2: %3 %4 %5 JMP $%6 ; Command $%7").arg(deviceName()).arg(address + static_cast<unsigned short>(offset), 4, 16, QChar('0')).arg(data[offset], 2, 16, QChar('0')).arg(data[offset + 1], 2, 16, QChar('0')).arg(data[offset + 2], 2, 16, QChar('0')).arg(static_cast<unsigned char>(data[offset + 1]) + (static_cast<unsigned char>(data[offset + 2]) << 8), 4, 16, QChar('0')).arg(command, 2, 16, QChar('0'));
          offset += 3;
          command++;
        }
    ...
    code.append(data);
    len = code.size();
    const auto codePtr {reinterpret_cast<unsigned char*>(code.data())};
    while (offset < len) {
      int lenOpCode = -1;
      if (drive1050) {
