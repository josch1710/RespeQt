...
namespace Printers {
  ...
  QChar BasePrinter::translateAtascii(const unsigned char b) const {
    return mAtascii(b);
  }
  ...
  void BasePrinter::setOutputWindow(OutputWindowPtr outputWindow) {
    if (mOutputWindow) {
      ...
      disconnect(this, &BasePrinter::setPrinterDimension, mOutputWindow.data(), &OutputWindow::updatePrinterDimension);
      mOutputWindow->close();
    }
    
    mOutputWindow = std::move(outputWindow);
    mOutputWindow->setSceneRect(getSceneRect());
    connect(this, &BasePrinter::addItem, mOutputWindow.data(), &OutputWindow::addItem);
    connect(this, &BasePrinter::setPrinterDimension, mOutputWindow.data(), &OutputWindow::updatePrinterDimension);
    emit setPrinterDimension(printerDimension());
    setupFont();
  }

  QRectF BasePrinter::getSceneRect() const {
    return {0, 0, 0, 0};
  }
  ...
  QByteArray BasePrinter::readDataFrame(const uint size, const bool isCommandFrame, const bool verbose) {
  ...
  void BasePrinter::dumpBuffer(const unsigned char *buf, const int len) {
  ...
  void BasePrinter::fillBuffer(char *line, const unsigned char *buf, const int len, const int ofs, const bool dumpAscii) { // NOLINT(*-convert-member-functions-to-static)
    *line = 0;
    if (len - ofs >= 16) {
      if (dumpAscii) {
      ...
      if (dumpAscii) {
        ...
        for (int i = 0; i < nbRemaining; i++) {
          if (buf[ofs + i] > 32 && buf[ofs + i] < 127) {
            ...
            line[57 + i] = static_cast<char>(buf[ofs + i] & 0x7F);
          }
        }
