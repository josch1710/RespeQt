...
void Atari1020::handleCommand(const quint8 command, const quint8 aux1,
                              const quint8 aux2)
{
    if (RespeqtSettings::instance()->printerEmulation() && mOutputWindow)
    {  // Ignore printer commands if Emulation turned OFF)
        switch (command)
        {
        ...
        switch (mCurrentCommand)
        {
            ...
            case 'R':  // move relative
            {
                ...
                if ((x >= 0) && (x <= 480))
                {
                    ...
                    {
                        case 'D': {
                            auto lineItem = new QGraphicsLineItem(mPenPoint.x(), mPenPoint.y(), point.x(), point.y());
