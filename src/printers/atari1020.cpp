...
namespace Printers {
  ...
  void Atari1020::handleCommand(const quint8 command, const quint8 aux1,
                                const quint8 aux2) {
    if (RespeqtSettings::instance()->printerEmulation() &&
        mOutputWindow) {// Ignore printer commands if Emulation turned OFF)
      switch (command) {
       ...
       * There is no byte in the status used to notify the Atari that the
       * printer is busy. When the printer is busy, it does not answer SIO
       * command. Sending a STATUS command to a busy 1020 results in a timeout.
       */
       ...
       */
          ...
          if (!RespeqtSettings::instance()->displayGraphicsInstructions()) {
            qDebug() << "!n"
                     << tr("[%1] Print (%2 chars)").arg(deviceName()).arg(len);
          }
  ...
  bool Atari1020::handleBuffer(const QByteArray &buffer, const unsigned int len) {
    /*
   * Characters and commands sent to the printer should be handled within
   * logical lines with the following structure <any sequence of printable
   * character, escape sequence and Graphics instructions> EOL This sequence may
   * be of any size. If the sequence is longer than 40 characters, it is sent by
   * the Atari using several 40-bytes buffers. All characters after an EOL are
   ...
   */
     ...
     * But if the printer is in Graphics mode, an unfinished command is executed
     * with default parameters. For example, if bytes 'C' and EOL are received
     * (change pen with no parameter), the command C0 is executed. Or if bytes
     * 'M' and EOL are received (move pen), the command M0,<current line> is
     * executed. EOL does not exit from Graphics mode. EOL can not be escaped,
     * meaning that Esc EOL will have the same effect as EOL.
     */
      if (b == 0x9B) {// EOL
        if (mGraphicsMode) {
      ...
      /*
     * clang-format off
     * Graphics mode is entered by an Esc sequence and terminated with the A
     * instruction or a STATUS command. Multiple Graphics instructions can be
     * sent in the same logical line using the '*' character. An instruction may
     * start in a physical buffer (in a previous SIO $53 command) and continue
     * in the next one. For example we can have:
     * 1st physical buffer: $1B $07
     *   M100,-100 ..... *C     for a total of 40 bytes
     * 2nd physical buffer:
     *   2*D300,-100 $9B
     * The intruction to change the pen color (C2) starts in the first buffer
     * and continues on the next one. Escape sequences are consumed but not
     * interpreted. The instruction P is used to print text. When activated, the
     * only way to get out of the text mode is using an EOL. After exiting
     * Graphics mode using the A instruction, the printer is still expecting
     * either an asterisk or an EOL to print plain ASCII text.
     *   $1B $07 M100,-100*C1*ATXT1*TXT2
     * In the above example, once *A has been executed,
     * the following characters (TXT1) are ignored until the asterisk is
     * encoutered and TXT2 is then printed. It means that, in Graphics mode, the
     * printer decodes one instruction with its parameters and then consumes all
     * other characters until the next asterisk or the next EOL.
     * clang-format on
     */
      if (mGraphicsMode) {
     ...
     * Changing the number of characters per line (20, 40 and 80) is only
     * possible at the start of a logical line.
     *
     * NOTE: Switching from standard character set to international one can
     * probably be done at any time but I did not manage to print any
     * international characters with my 1020. The following implementation
     * assumes it switching works at any time.
     */
      else {

        /*
       * If Esc is found at the start of a logical line, then the escape
       * sequence is interpreted. Any number of consecutive Esc chars at the
       * start of the logical line are interpreted as only one Esc. If Esc char
       * is found at the start of a buffer which is not the start of the logical
       * line, it is ignored. By default, the next character after an escape
       * always cancels the ESCAPE state. If this character is also an Esc, the
       * printer enters again in ESCAPE state. It means that, in a middle of a
       * logical line, all repeated Esc chars and the next non Esc char are
       * ignored: Esc X or Esc Esc X or Esc Esc Esc X: all these sequences are
       * ignored. The ESCAPE state is local to a logical line. It is reset when
       * EOL is encountered.
       */
        if (mEsc) {
          mEsc = false;
          switch (b) {
            ...
            case 0x1B:// Cancel Esc
              if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
                qDebug() << "!n"
                         << tr("[%1] Escape character repeated").arg(deviceName());
              }
              mEsc = true;
              break;
            ...
            case 0x17:// CTRL+W: Enter international mode
              if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
                qDebug()
                        << "!n"
                        << tr("[%1] Entering international mode").arg(deviceName());
              }
              mInternational = true;
              break;
            ...
            case 0x18:// CTRL+X: Exit international mode
              if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
                qDebug() << "!n"
                         << tr("[%1] Exiting international mode").arg(deviceName());
              }
              mInternational = false;
              break;

            default:// Other commands are ignored unless at the start of a logical
                    // line

              /*
           * Esc commands are only interpreted at the start of the buffer except
           * for International character set. For example, sending AB $1B $07 C1
           * to the printer while in text mode will display AB but it won't
           * enter Graphics mode and won't change the pen color to blue. Again,
           * sending AB $1B $13 CD to the printer while in text mode will
           * display ABCD in 40 column mode even if an Esc Ctrl-S (80-column
           * mode) is found between AB and CD.
           */
              if (!mStartOfLogicalLine) {
                if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
                  qDebug() << "!n"
                           << tr("[%1] Escape character not on start of line")
                                      .arg(deviceName());
                }
              } else {
                switch (b) {
                  ...
                  case 0x07:// CTRL+G: Enter Graphics Mode
                    if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
                      qDebug() << "!n"
                               << tr("[%1] Enter Graphics mode").arg(deviceName());
                    }
                    mStartOfLogicalLine = false;
                    mGraphicsMode = true;
                    resetGraphics();
                  ...
                  case 0x0E:// CTRL+N: 40 characters
                    if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
                      qDebug() << "!n"
                               << tr("[%1] Switch to 40 columns").arg(deviceName());
                    }
                  ...
                  case 0x10:// CTRL+P: 20 characters
                    if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
                      qDebug() << "!n"
                               << tr("[%1] Switch to 20 columns").arg(deviceName());
                    }
                  ...
                  case 0x13:// CTRL+S: 80 characters
                    if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
                      qDebug() << "!n"
                               << tr("[%1] Switch to 80 columns").arg(deviceName());
                    }
                    // mOutput->calculateFixedFontSize(80);
                    break;
                  ...
                  case 0x17:// CTRL+W: Enter international mode
                    if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
                      qDebug()
                              << "!n"
                              << tr("[%1] Enter international mode").arg(deviceName());
                    }
                    mInternational = true;
                    break;
                  ...
                  case 0x18:// CTRL+X: Exit international mode
                    if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
                      qDebug()
                              << "!n"
                              << tr("[%1] Exit international mode").arg(deviceName());
                    }
                    mInternational = false;
                    break;
  ...
  bool Atari1020::checkGraphicsCommand(const unsigned char b) {
    for (int i = 0; i < (int) (sizeof(ALLOWED_GRAPHICS_COMMANDS) /
                               sizeof(struct GRAPHICS_COMMAND));
         i++) {
   ...
   * This is the same purpose as the EOL. It executes the current graphical
   * command even if not complete. For example M120* will move the printer head
   * to X=120 and Y=0 because Y has not been specified. If the last command was
   * P (for Print text) then the asterisk is not a marker but a simple char to
   * print.
   */
    if ((mAutomataState != AUTOMATA_TEXT) &&
        (b == '*')) {// end of Graphics instruction
      executeGraphicsCommand();
      resetGraphics();
      return;
    }
    ...
    switch (mAutomataState) {
      case AUTOMATA_START:
        if (!checkGraphicsCommand(b)) {
          if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
            qDebug() << "!n"
                     << tr("[%1] Unknown Graphics command $%2")
                                .arg(deviceName())
                                .arg(b, 2, 16, QChar('0'));
          }
    ...
    if (mCurrentCommand) {
      switch (mCurrentCommand) {
        ...
        case 'H':// set to HOME
          mPenPoint.setX(0);
          mPenPoint.setY(0);
          qDebug() << "!n" << tr("[%1] Move to Home").arg(deviceName());
          break;
        ...
        case 'S':// Scale characters
        {
          ...
          if (scale >= 0 && scale <= 63) {
            ...
            if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
              qDebug()
                      << "!n"
                      << tr("[%1] Scale characters to %2").arg(deviceName()).arg(scale);
            }
          } else {
            if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
              qDebug()
        ...
        case 'L':// Line mode 0 = solid, anything else dashed
        {
          ...
          if (line >= 0 && line <= 15) {
            ...
            if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
              qDebug() << "!n"
                       << tr("[%1] Set line mode command ignored (%2 should be in "
                             "range 0-15)")
                                  .arg(deviceName(), QString(mFirstNumber));
            }
          }
            ...
            switch (mCurrentCommand) {
              case 'D':
                executeGraphicsPrimitive(
                        new GraphicsDrawLine(mPenPoint, mPen, point));
                if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
                  qDebug() << "!n"
                           << tr("[%1] Draw to point (%2,%3)")
                                      .arg(deviceName())
                                      .arg(x)
                                      .arg(y);
                }
              ...
              case 'J':
                ...
                if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
                  qDebug() << "!n"
                           << tr("[%1] Draw relative to point (%2,%3)")
                                      .arg(deviceName())
                                      .arg(x)
                                      .arg(y);
                }
              ...
              case 'R':
                if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
                  qDebug() << "!n"
                           << tr("[%1] Move relative to point (%2,%3)")
                                      .arg(deviceName())
                                      .arg(x)
                                      .arg(y);
                }
        ...
        case 'X':// draw axes 0 = Y-axis, otherwise X-axis
        {
          ...
          if (xAxe) {
            if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
              qDebug() << "!n"
                       << tr("[%1] Draw X-axis with size %2 and %3 marks")
                                  .arg(deviceName())
                                  .arg(size)
                                  .arg(count);
            }
            ...
            if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
              qDebug() << "!n"
                       << tr("[%1] Draw Y-axis with size %2 and %3 marks")
                                  .arg(deviceName())
                                  .arg(size)
                                  .arg(count);
            }
        ...
        case 'Q':// Set text orientation
        {
          ...
          if (orientation >= 0 && orientation <= 3) {
            ...
            if (RespeqtSettings::instance()->displayGraphicsInstructions()) {
              qDebug() << "!n"
                       << tr("[%1] Set text orientation to %2°")
                                  .arg(deviceName())
  ...
  bool Atari1020::drawAxis(bool xAxis, int size, int count) {
    ...
    for (int c = 1; c <= count; c++) {
      if (xAxis) {
        ...
        executeGraphicsPrimitive(new GraphicsDrawLine(
                QPoint(xc, mPenPoint.y() - 2), pen, QPoint(xc, mPenPoint.y() + 2)));
      } else {
        auto yc = mPenPoint.y() + c * size;
        executeGraphicsPrimitive(new GraphicsDrawLine(
                QPoint(mPenPoint.x() - 2, yc), pen, QPoint(mPenPoint.x() + 2, yc)));
      }
    }
  ...
  bool Atari1020::drawText() {
    executeGraphicsPrimitive(new GraphicsDrawText(
            mPenPoint, mPen, mTextOrientation, mFont, QString(mPrintText)));
